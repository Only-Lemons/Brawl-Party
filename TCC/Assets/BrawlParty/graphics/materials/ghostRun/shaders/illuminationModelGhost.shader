Shader "myShader/illumationModelGhost"
{
	Properties
	{
		...
		_Roughness ("Roughness", Range(0.001, 10)) = 2
	}
		SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				diffuseLight *= _Color.rgb * tex2D(_MainTex, uv);

				//Direct Spec Light
				float3 camPos = _WorldSpaceCameraPos;
				float3 fragToCam = normalize(camPos - i.worldPos); // viewDir

				float3 viewReflect = reflect(-fragToCam, normal);
				float specFalloff = max(0,dot(viewReflect, lightDir));
				specFalloff = (pow(specFalloff, _Gloss) / _Roughness);

				float3 finalSurfColor = diffuseLight + specFalloff; 


				return float4(finalSurfColor, 1);
			}
